...
bot.use(session({
  initial: () => ({ currentMenu: "main", selectedLeader: null, previousMenu: null })
}));
...
function getRandomNPC(era) { return npcList[Math.floor(Math.random() * npcList.length)]; }
...
function getShopGlassKeyboard() {
  return new InlineKeyboard()
    .text("🔙 بازگشت به کاخ", "back_to_game");
}
...
bot.command("start", async (ctx) => {
  ...
  await ctx.replyWithPhoto(categoryImages.ancient, {
    ...
    parse_mode: "Markdown",
    reply_markup: getMainGlassKeyboard()
  });
});
...
async function selectLeader(ctx, leaderKey) {
  ...
  await ctx.replyWithPhoto(leader.image, {
    ...
    parse_mode: "Markdown",
    reply_markup: getGameGlassKeyboard()
  });
}
...
async function showShop(ctx) {
  ...
  if (!user) {
    await ctx.reply("❌ ابتدا یک رهبر انتخاب کن!", { reply_markup: getMainGlassKeyboard() });
    return;
  }
...
async function startBattle(ctx) {
  ...
  await ctx.replyWithAnimation(anim, {
    ...
    parse_mode: "Markdown",
    reply_markup: new InlineKeyboard().text("⚔️ میدان رزم دوباره", "battle").text("🪞 بازارچه شیشه‌ای", "open_shop").row().text("🔙 بازگشت به کاخ", "back_to_game")
  });
}
  ...
  try {
    ...
    if (data.startsWith("cat_")) await showCategory(ctx, data.replace("cat_", ""));
      ...
      if (user) {
        ...
        await ctx.reply("❌ خطا! /start رو بزن.", { reply_markup: getMainGlassKeyboard() });
      }
      ...
      if (user && item && user.gold >= item.price) {
        ...
        await ctx.reply(`❌ دینار کافی نیست! نیاز به ${item.price - user.gold} دینار بیشتر دارید.`, { reply_markup: getBackKeyboard() });
      }
    }
  } catch (error) {
...
bot.start();
console.log("🎮 بازی بقای باستانی - نسخه شیشه‌ای کامل روشن شد...");